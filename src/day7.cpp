...
auto aoc::day7::find_smallest_sufficient(std::vector<unsigned int> &sums, unsigned threshold) -> unsigned {
...
auto aoc::day7::sum_if(std::vector<unsigned int> &sums, unsigned threshold) -> unsigned {
...
auto aoc::day7::Directory::add(Directory &&dir) -> std::shared_ptr<Directory> {
    auto name = dir.name();
...
auto aoc::day7::Directory::parent() -> std::shared_ptr<Directory> {
    return parent_;
}

auto aoc::day7::Directory::get_subdirectory(const std::string &title) -> std::optional<std::shared_ptr<Directory>> {
...
auto aoc::day7::Directory::get_file(const std::string &title) -> std::optional<std::shared_ptr<File>> {
    if (!entries_.contains(title)) return std::nullopt;
    auto item = entries_.at(title);
...
aoc::day7::Directory::Directory(std::shared_ptr<Directory> parent, const std::string &name):
    TreeObject(0, name), parent_(parent), entries_{} {}

auto aoc::day7::Directory::add(std::shared_ptr<Directory> parent, std::istream &is) -> void{
...
auto aoc::day7::Directory::get_subdirectories() const -> std::vector<std::shared_ptr<Directory>> {
 ...
 */
static void calculate_sums(const std::shared_ptr<aoc::day7::Directory>& dir, std::vector<unsigned>& sums){
...
auto aoc::day7::Directory::get_recursive_subdirectory_sizes() const -> std::vector<unsigned> {
...
auto aoc::day7::process_command_line(const std::string &line, DirectoryTree &tree) -> void {
...
auto aoc::day7::DirectoryTree::cd(const std::string &dir) -> void {
    if (dir == "/") {
        current_ = root_;
        return;
    }
...
auto aoc::day7::DirectoryTree::current() -> std::shared_ptr<Directory> {
    return current_;
}

auto aoc::day7::DirectoryTree::root() -> std::shared_ptr<Directory> {
    return root_;
}

aoc::day7::DirectoryTree::DirectoryTree(const Directory &root): root_(std::make_shared<Directory>(root)) {
    current_ = root_;
}
