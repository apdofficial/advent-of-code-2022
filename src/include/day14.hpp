...
struct Trace {
  ...
  friend auto operator>>(std::istream &is, Trace &trace) -> std::istream & {
    ...
    while (iss >> col >> delimiter >> row) {
      if (row == row_prev) {
        for (int col_ : std::views::iota(std::min(col_prev, col),
                                         std::max(col_prev, col) + 1)) {
          trace.path_.emplace_back(row, col_);
        }
      }
      if (col == col_prev) {
        for (int row_ : std::views::iota(std::min(row_prev, row),
                                         std::max(row_prev, row) + 1)) {
  ...
  friend auto operator<<(std::ostream &os, const Trace &trace)
      -> std::ostream & {
...
struct Scan {
  ...
  friend auto operator<<(std::ostream &os, Scan &scan) -> std::ostream & {
    const auto rows = std::views::iota(scan.sand_bounding_.min_row,
                                       scan.sand_bounding_.max_row + 1);
    const auto cols = std::views::iota(scan.sand_bounding_.min_col,
                                       scan.sand_bounding_.max_col + 1);
  ...
  static inline Coordinates default_start{0,500};

  [[nodiscard]] auto bounding() const -> auto & { return rocks_bounding_; }
...
[[nodiscard]] auto parse_input(std::istream &stream, bool print = false)
    -> Scan;
