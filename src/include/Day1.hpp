...
namespace aoc {
    template<class T>
    class Day1: public Day<T>{
    public:
        static auto compute_calorie_sums(std::span<std::optional<int>> calories) ->  std::vector<int>{
        ...
        // T(n) = O(n)
        static auto compute_top_1_elf_calories(std::span<std::optional<int>> calories) -> std::optional<int>
        {
            auto elf_groups = calories | std::ranges::views::split(std::nullopt);
            int max_calories = 0;
        ...
        // T(n) = O(n*m + n^2)
        static auto compute_top_3_elf_calories_bubble(std::span<std::optional<int>> calories) -> std::optional<int>
        {
...
//            assert(n >= 3); // at least 3 elves are needed
        ...
        static auto compute_top_3_elf_calories_quick(std::span<std::optional<int>> calories) -> std::optional<int>
        {
//            assert(calories.size() >= 3 + 2); // at least 3 calorie entries for each elf and 2 std::nullopt elf separators
            std::vector<int> sums = compute_calorie_sums(calories);
            auto n = sums.size();
//            assert(n >= 3); // at least 3 elves are needed
        ...
        // m = number of calories per elf
        // T(n) = O(n*m)
        static auto compute_top_3_elf_calories_inplace(std::span<std::optional<int>> calories) -> std::optional<int>
        {
            int max1 = 0;
            int max2 = 0;
            int max3 = 0;
        ...
        void on_new(std::string_view line) override;

        void on_lines_completed() override;
    };
